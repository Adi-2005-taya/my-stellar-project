...
export const CONTRACT_ADDRESS =
  "CDZ5UWUWW47F4L2R3NSWXNU52QNYFD3FWBBZWAEI5ZMJ3NRARHQJDSZX";
...
// ============================================================
// Digital Asset Inheritance — Contract Methods
// ============================================================

/**
 * Create an inheritance plan.
 * Calls: create_plan(owner: Address, plan_id: String, unlock_time: u64)
 */
export async function createPlan(
  ...
  return callContract(
    "create_plan",
    [toScValAddress(caller), toScValString(planId), toScValU64(unlockTime)],
    caller,
    true
  );
}
 ...
 * Calls: add_beneficiary(owner: Address, plan_id: String, beneficiary: Address)
 */
export async function addBeneficiary(
  ...
  return callContract(
    "add_beneficiary",
    [toScValAddress(caller), toScValString(planId), toScValAddress(beneficiary)],
    caller,
    true
  );
}
...
/**
 * Add an asset to an inheritance plan.
 * Calls: add_asset(owner: Address, plan_id: String, token_address: Address, amount: i128)
 */
export async function addAsset(
  ...
  return callContract(
    "add_asset",
    [toScValAddress(caller), toScValString(planId), toScValAddress(tokenAddress), toScValI128(amount)],
    caller,
    true
  );
}
...
/**
 * Claim assets from an inheritance plan (beneficiary only).
 * Calls: claim(caller: Address, plan_id: String)
 */
export async function claimAssets(
  ...
  return callContract(
    "claim",
    [toScValAddress(caller), toScValString(planId)],
    caller,
    true
  );
}
...
/**
 * Cancel an inheritance plan (owner only).
 * Calls: cancel_plan(owner: Address, plan_id: String)
 */
 ...
 * Calls: get_plan(plan_id: String) -> Plan
 */
export async function getPlan(
  ...
  return readContract(
    "get_plan",
    [toScValString(planId)],
    caller
  );
}

/**
 * Get all plan IDs (read-only).
 * Calls: get_all_plan_ids() -> Vec<String>
 */
